...
The adapter runs one app-owned polling task while authenticated, targeting a 500 ms refresh cadence regardless of UI visibility, subscribers, or playback state. Request duration is subtracted from the next sleep; slow requests cannot overlap and can exceed that target. HTTP 429 delays requests until Retry-After expires. Disconnect, shutdown, or loss of authorization end polling. Transient errors preserve the last media snapshot and expose an issue instead of removing the flanks. Overlapping commands are rejected. A command triggers a fresh observation. A pending seek is separate presentation intent: it holds the dragged value through stale observations, then yields to a matching provider confirmation, a track change, a command error, or an authoritative sample at least ten seconds later. The UI reflects device/action capabilities and reports server rejections.

Queue/repeat/shuffle provider capabilities are retained, but the debug-pass transport row displays only previous, play/pause, and next as requested. Unsupported source types produce no empty player. The state preserves provider track identity for a future licensed lyrics provider; no real lyrics or scraping are implemented.
...
`SystemAudioMeter` uses public ScreenCaptureKit with only an `.audio` output, 48 kHz stereo, and current-process audio excluded. No video output is registered, frames are never consumed/rendered, microphone capture is disabled, and samples are never saved. A serial audio queue runs a 2,048-sample Hann-windowed vDSP FFT. Stereo powers combine after transformation, preserving opposite-phase signals. Seven bands (60–120, 120–250, 250–500, 500–1,000, 1,000–2,000, 2,000–5,000, 5,000–12,000 Hz) map log energy to 0.12–1.0 with fast attack and slower release. Publications are limited to 30 Hz using uptime. The view has no timer or synthetic oscillator and uses seven fixed bar identities. System audio includes other apps; this is not isolated Spotify audio.
