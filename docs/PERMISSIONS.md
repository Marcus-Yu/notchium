...
| System Audio Recording | Spotify playback starts the real waveform, or the user explicitly retries it after denial | `NSAudioCaptureUsageDescription`; Spotify-scoped Core Audio process tap; no microphone key/entitlement because microphone input is not used | TCC system-audio capture | First tap prompts through macOS | Same privacy flow; signing/sandbox and review acceptance must be validated before Store-profile inclusion | Media metadata, artwork, progress, controls, and static waveform fallback | System Settings → Privacy & Security → Screen & System Audio Recording; stop before guiding to Settings; retry with Enable System Audio Waveform | Stop IOProc, destroy aggregate device and process tap, release in-memory PCM, and restore static bars | Allow/deny; revoke while active; Spotify absent/quit/relaunch; tap timeout/failure; sleep/wake; no disk writes; no mic prompt; unrelated-app audio isolation; Store-profile capability gate |
| Keyboard suppression trust | User explicitly clicks Keyboard Lock | Public session-level, head-insert, active `CGEventTap`; Accessibility trust is requested with the public AX API; no usage-description key; never root/HID | TCC Accessibility trust | Feature enters locked state only after trust, tap creation, run-loop installation, enablement, and `CGEvent.tapIsEnabled` verification succeed. The native request is attempted at most once per installation; System Settings opens only after the user clicks the in-app button | High review risk; compiled out or unavailable unless sandbox and review validation pass | Keyboard remains fully usable; mouse and trackpad are never suppressed | System Settings → Privacy & Security → Accessibility; recheck on the next explicit lock attempt | A tap disabled by timeout or user input is re-enabled and verified; failed recovery unlocks, tears down the tap, and explains why. Holding Command–Option–Escape for two monotonic seconds is consumed by the tap and unlocks; an early release cancels progress | First request; trust denied; tap creation failure; tap enablement failure; tap disabled by timeout/user input; Secure Input; revoke during lock; mouse pass-through; click unlock; Command–Option–Escape hold/cancel; crash/termination; Store profile excluded |
...
| Caffeine | Ordinary idle-sleep assertions require no permission. Optional [closed-lid mode](LID_AWAKE.md) requires administrator approval of its SMAppService daemon; denied or unavailable helpers leave it off. |
| Keyboard Lock | The app never presents itself as locked unless the active session tap and effective keyboard mask are confirmed. Secure Input blocks activation and ends an existing lock with specific guidance. Failure means unlocked. |
...
Stage 5 Calendar requests full access at first app launch so upcoming events can appear without a separate setup step. System Audio Recording, Camera, direct Screen Capture, Notifications, Keyboard Lock trust, Apple Music, Spotify, Safari extension access, and each Chromium browser adapter are introduced and requested separately at point of use.
